// src/sections/edukasi_interaktif/HeaderKuisSection.jsx
...
const HeaderKuis = () => {
  ...
  // dipanggil dari AddQuizModal saat submit
  const handleAddQuiz = async (formData) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/kuis", data, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      // kalau sukses: tutup form kuis, tampilkan popup sukses (admin/Success.jsx)
    ...
    <>
      <div className="w-full h-[400px] bg-linear-to-r from-[#FF9D01] to-[#FFFFFF]/80  relative">
        <div className="absolute top-0 left-0 h-[400px] flex items-center pb-10 ">
          <img
            src="src/assets/game controller.png"
            className="w-[500px] mix-blend-color-burn"
            alt=""
          />
        </div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex text-3xl font-bold flex-col items-center gap-y-4">
          <div>
            <div>
              Kuis & Game{" "}
              <span className="bg-[#FF9500] bg-clip-text text-transparent drop-shadow-md">
                Interaktif
              </span>
            </div>
          </div>
          <div className="flex max-w-[55%] text-sm font-normal text-[#757570] text-center flex-col items-center font-sans whitespace-nowrap">
            <div>
              Uji pengetahuan siswa tentang berbagai mata pelajaran dengan kuis
              dan game interaktif
            </div>
            <div className="mt-4 flex items-center gap-x-3">
              ...
              <div>
                <img src="src/assets/wayground.png" alt="" />
              </div>
            </div>
          </div>
        ...
        <div className="absolute top-0 right-0 h-[400px] flex items-center pt-2 pr-20">
          <img
            ...
            className="w-[350px] mix-blend-color-burn"
            alt=""
          />
        </div>
      ...
      {/* Popup sukses / gagal pakai Success.jsx & Failed.jsx admin */}
      <SuccessModal
