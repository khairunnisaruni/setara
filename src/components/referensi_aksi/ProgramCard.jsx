...
export default function ProgramCard({ program }) {
  return (
    <div className="bg-white rounded-2xl shadow hover:shadow-lg overflow-hidden flex flex-col">
      <img
        src={program.image}
        alt={program.judul}
      ...
      <div className="p-5 flex flex-col grow">
        <div className="flex items-center justify-between mb-2">
          {/* Kategori + icon */}
          <div className="flex items-center gap-1 text-sm font-semibold text-[#FF9500]">
          ...
          {/* Status */}
          <span className="text-[#1E40AF] bg-[#DBEAFE] px-2 py-1 rounded-full text-xs">
            {program.status}
          </span>
        </div>

        <h3 className="text-lg font-semibold mb-1">{program.judul}</h3>
        <p className="text-sm text-gray-900 mb-2">{program.penyelenggara}</p>
        <p className="text-sm text-gray-500 grow">{program.deskripsi}</p>

        <div className="mt-4 text-sm space-y-1">
          <p className="font-medium text-[#317B74]">Periode: {program.periode}</p>
          <p>
            <span className="bg-[#FFE5FE] text-[#8A38F5] px-2 py-0.5 rounded-full text-xs font-medium">
              Deadline:{" "}
              {program.deadline}
            </span>
          </p>
        ...
        <div className="flex gap-2 mt-4">
          <button className="w-[118px] h-10 border border-[#317B74] text-[#317B74] hover:bg-[#317B74] hover:text-white rounded-lg text-sm">
            Lihat Detail
          </button>
          <a
            href={program.link}
            ...
            className="w-[174px] h-10 flex items-center justify-center bg-[#FF9D01] hover:bg-[#e88a00] text-white rounded-lg text-sm gap-1"
          >
            Kunjungi Sumber
            {/* nanti tambahkan SVG icon di sini, ukuran 16x16 */}
          </a>
        </div>
      </div>
    </div>
